...
float32 lambda_r =      125.0;
float32 lambda_e =      500.0;
float32 K11;
float32 K12;
float32 K2;
float32 L1;
...
inline void setAverageLegVoltages(float Vdc, float u) {
    ...
    if(u >= 0) {
        thetaV = 1.57079632679;
    } else {
        thetaV = -1.57079632679;
    }

    desiredLegVoltages[0] = .5 * Vdc + 0.47140452079 * alphaV * cos(N * rotorAngle + thetaV - 0.52359877559);
    desiredLegVoltages[1] = .5 * Vdc + 0.47140452079 * alphaV * cos(N * rotorAngle + thetaV - 2.61799387799);
    desiredLegVoltages[2] = .5 * Vdc + 0.47140452079 * alphaV * cos(N * rotorAngle + thetaV + 1.57079632679);
