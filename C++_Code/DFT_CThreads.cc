...
void threadDftHoriz(struct horizThread_data *threadarg)
{
...
void threadDftVert(struct vertThread_data *threadarg)
{
...
Complex *doDft(Complex *data, int width, int height, bool forward = true)
{
    ...
    for (int iH = 0; iH < height; iH++)
    {
        ...
        horizThread[iH] = std::thread(threadDftHoriz, &tdh[iH]);
        // if (rc) {
    ...
    for (int iW = 0; iW < width; iW++)
    {
        ...
        vertThread[iW] = std::thread(threadDftVert, &tdv[iW]);
        if (rc)
        {
            std::cout << "<<< ERROR: Cannot create thread! >>>" << std::endl;
            exit(-1);
